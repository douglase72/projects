...
public interface PersonCommandRepository {
  ...
  ///
  /// @param publicId the catalogue id of the person to remove
  void deleteByPublicId(PersonPublicId publicId);
  
  /// Loads a person by catalogue id, for editing.
  ///
  ...
  /// Loads a person by natural id — the lookup that decides whether ingestion
  /// inserts or updates.
  ///
  /// @param sourceId the natural id from the upstream catalogue
  /// @return the aggregate, or empty if the person has not been ingested
  Optional<Person> findBySourceId(SourceId sourceId);
}