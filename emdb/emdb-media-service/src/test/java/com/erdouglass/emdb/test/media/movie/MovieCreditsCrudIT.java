...
class MovieCreditsCrudIT extends AbstractTest {
  private static final Logger LOGGER = Logger.getLogger(MovieCreditsCrudIT.class);
  ...
  void testCreateMovie() throws IOException, InterruptedException {
    ...
    var request = HttpRequest.newBuilder().uri(UriBuilder.fromUri(MOVIES_URL).build())
        .POST(HttpRequest.BodyPublishers.ofString(OBJECT_MAPPER.writeValueAsString(movieRequest))).build();
  ...
  @Test
  @Order(2)
  void testAddMikeMyersCredit() throws IOException, InterruptedException {
    var createRequest = List.of(
    ...
    var request = HttpRequest.newBuilder()
        .uri(UriBuilder.fromUri(MOVIES_URL).path(movieId.toString()).path("credits").build())
        .PUT(HttpRequest.BodyPublishers.ofString(OBJECT_MAPPER.writeValueAsString(createRequest)))
        .build();
    long startTime = System.nanoTime();
    var response = HTTP_CLIENT.send(request, BodyHandlers.ofString());
    long et = TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - startTime);
    var credits = OBJECT_MAPPER.readValue(response.body(), new TypeReference<List<ResponseDto>>() {})
        .stream().collect(Collectors.toMap(ResponseDto::tmdbId, Function.identity()));
    assertEquals(207, response.statusCode());
    assertEquals(201, credits.get(12073).status());
    LOGGER.infof("Created 1 credit in: %d ms", et);
  }
  
  @Test
  @Order(3)
  void testReplaceCredits() throws IOException, InterruptedException {
    var createRequest = List.of(
        MovieCreditCreateDto.builder()
        ...
        .person(PersonCreateDto.builder()
            ...
            .tmdbId(13922)
            .gender(Gender.MALE)
            .profile("/1KSM8u8vBYCYybMOFxythVJPNqD.jpg")
      ...
      MovieCreditCreateDto.builder()
        ...
        .person(PersonCreateDto.builder()
            ...
            .profile("/m18Mx9Fj9XKA9CTDGBDUhluSuGt.jpg")
    ...
    var credits = OBJECT_MAPPER.readValue(response.body(), new TypeReference<List<ResponseDto>>() {})
        .stream().collect(Collectors.toMap(ResponseDto::tmdbId, Function.identity()));
    assertEquals(207, response.statusCode());
    assertEquals(201, credits.get(14386).status());
    assertEquals(201, credits.get(13922).status());
    LOGGER.infof("Created 2 credits in: %d ms", et);
  }
  
  @Test
  @Order(4)
  void testReplaceCreditsAgain() throws IOException, InterruptedException {
    var createRequest = List.of(
        MovieCreditCreateDto.builder()
          .tmdbId("52fe427bc3a36847f8022183")
          .type(CreditType.CAST)
    ...
    assertEquals(207, response.statusCode());
    assertEquals(201, credits.get(12073).status());
    assertEquals(200, credits.get(14386).status());
    assertEquals(200, credits.get(13922).status());
    LOGGER.infof("Created 3 credits in: %d ms", et);
  }
