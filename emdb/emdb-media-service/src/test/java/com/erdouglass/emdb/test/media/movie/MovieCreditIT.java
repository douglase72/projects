...
class MovieCreditIT extends AbstractTest {
  private static final Logger LOGGER = Logger.getLogger(MovieCreditIT.class);
  ...
  @Test
  @Order(2)
  void testUpdateMovieCredit() throws IOException, InterruptedException {
    ...
    var command = UpdateMovieCredit.of("Test Character", 3);
    request = HttpRequest.newBuilder().uri(UriBuilder
        .fromUri(MOVIES_URL).path(CREDITS).path(creditId.toString()).build())
