...
class PeopleCrudIT extends AbstractTest {
  private static final Logger LOGGER = Logger.getLogger(PeopleCrudIT.class);
  private static final String BATCH = "batch"; 
  
  /// This should result in two insert statements.
  ///
  /// @throws IOException
  ...
  /// This should result in a single insert and a single update statement.
  ///
  /// @throws IOException
  /// @throws InterruptedException
  @Test
  @Order(2)
  void testUpdatePeople() throws IOException, InterruptedException {
    var commands = List.of(
        SavePerson.builder()
          .tmdbId(148815)
          .name("Harrison Ford")
          ...
          .profile(UUID.fromString("c7cbf367-906a-4058-aa0e-83fc914c7979"))
    ...
    long startTime = System.nanoTime();
    var response = HTTP_CLIENT.send(request, BodyHandlers.ofString());
    var savedPeople = OBJECT_MAPPER.readValue(response.body(), new TypeReference<List<EmdbResponse>>() {})
        .stream().collect(Collectors.toMap(EmdbResponse::tmdbId, Function.identity()));
    long et = TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - startTime);
    assertEquals(207, response.statusCode());
    assertEquals(200, savedPeople.get(3).status());
    assertEquals(204, savedPeople.get(13918).status());
    assertEquals(201, savedPeople.get(148815).status());
    LOGGER.infof("Updated %d people in %d ms", commands.size(), et);
  }  
