...
/// A base abstract class for all entities requiring a primary key, business key
/// (`tmdbId`), and auditing timestamps.
///
/// Equality is determined by the natural business key ({@code tmdbId}), not the
/// surrogate primary key ({@code id}).
///
...
public abstract class BasicEntity<T> {
  ...
  /// The natural business key derived from The Movie Database (TMDB). This is
  /// immutable and determines the Java identity of the entity.
  @NotNull
  ...
  ///
  /// This implementation uses the natural business key ({@code tmdbId}) rather than the 
  /// surrogate {@code id}. This allows for comparison of transient (unsaved) entities.
  ///
