package com.erdouglass.emdb.media.movie.adapter.out.stub;

import java.util.Map;
import java.util.Set;
import java.util.stream.Collectors;

import jakarta.enterprise.context.ApplicationScoped;
import jakarta.inject.Inject;

import com.erdouglass.emdb.media.kernel.PublicId;
import com.erdouglass.emdb.media.kernel.TmdbId;
import com.erdouglass.emdb.media.movie.application.port.out.PersonStub;
import com.erdouglass.emdb.media.movie.application.port.out.ResolvePersonStub;
import com.erdouglass.emdb.media.person.application.port.in.ResolvePersonCommand;
import com.erdouglass.emdb.media.person.application.port.in.ResolvePersonCommand.Reference;
import com.erdouglass.emdb.media.person.application.port.in.ResolvePersonUseCase;

@ApplicationScoped
class ResolvePersonStubAdapter implements ResolvePersonStub {
  
  @Inject
  ResolvePersonUseCase resolver;

  @Override
  public Map<TmdbId, PublicId> resolve(Set<PersonStub> stubs) {
    var command = ResolvePersonCommand.of(stubs.stream()
        .map(c -> Reference.of(c.tmdbId(), c.name()))
        .collect(Collectors.toSet()));
    return resolver.resolve(command);
  }
}
