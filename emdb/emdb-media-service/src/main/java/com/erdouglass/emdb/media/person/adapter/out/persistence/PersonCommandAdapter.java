...
class PersonCommandAdapter implements PersonCommandRepository {
  ...
  public List<Person> insertAll(List<Person> people) {
    return repository.insertAll(people.stream().map(this::toPersonEntity).toList()).stream()
        .map(this::toPerson)
        .toList();
  }
  ...
  public Optional<Person> findById(PublicId id) {
    return repository.findById(id.value()).map(this::toPerson);
  }
  ...
  public List<Person> findByTmdbIdIn(List<TmdbId> tmdbIds) {
    return repository.findByTmdbIdIn(tmdbIds.stream().map(TmdbId::value).toList()).stream()
        .map(this::toPerson)
        .toList();
  }
