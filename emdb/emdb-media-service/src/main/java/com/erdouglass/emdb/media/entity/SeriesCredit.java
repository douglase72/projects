...
public class SeriesCredit extends Credit {
  ...
  public boolean isEqualTo(SeriesCredit other) {
    ...
    boolean fieldsMatch = Objects.equals(this.order().orElse(null), other.order().orElse(null))
        && Objects.equals(this.totalEpisodes, other.totalEpisodes);
    if (!fieldsMatch) return false;
    if (this.roles.size() != other.roles().size()) {
      return false;
    }
    for (int i = 0; i < this.roles.size(); i++) {
      if (!this.roles.get(i).isEqualTo(other.roles().get(i))) {
        return false;
      }
    }
    return true; 
  ...
  public void roles(List<Role> roles) {
    ...
    this.roles.forEach(r -> r.seriesCredit(this));
  }
  ...
  public void series(Series series) {
    this.series = series;
  }
  ...
  public void totalEpisodes(Integer totalEpisodes) {
