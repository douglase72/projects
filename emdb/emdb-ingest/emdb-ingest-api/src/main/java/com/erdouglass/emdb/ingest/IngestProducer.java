...
///
/// Each published message is tagged with a freshly generated correlation id
/// and an event-type header so downstream consumers and logging decorators
/// can trace the ingest end-to-end.
@ApplicationScoped
public class IngestProducer {
  ...
  ///
  /// The correlation id is attached as RabbitMQ metadata and also returned
  /// to the caller so the originator of the request can correlate the
  ...
  public UUID publish(@NotNull @Valid final IngestMedia command) {
