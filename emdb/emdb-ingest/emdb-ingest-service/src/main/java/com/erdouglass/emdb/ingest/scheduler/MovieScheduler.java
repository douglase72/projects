...
/// Produces the [IngestMedia] command for movies according to schedule.
///
/// Runs on a virtual thread at the cron expression configured by
/// `emdb.movie.scheduler`. Concurrent executions are skipped so a long-running
...
class MovieScheduler {
  ...
  /// Looks up movies changed in the last 24 hours and publishes an
  ...
  public void execute() {
    ...
    for (var tmdbId : changes) {
      var command = IngestMedia.of(tmdbId, IngestType.MOVIE, IngestSource.SCHEDULER);
