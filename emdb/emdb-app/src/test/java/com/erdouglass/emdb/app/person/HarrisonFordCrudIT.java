...
import com.erdouglass.emdb.media.person.adapter.in.rest.PersonResponse;
...
private static final Logger LOGGER = Logger.getLogger(HarrisonFordCrudIT.class);
  ...
  void testSavePerson() throws IOException, InterruptedException {
    ...
    var result = TestHelper.OBJECT_MAPPER.readValue(response.body(), PersonResponse.class);
    personId = result.id();
    LOGGER.infof("Saved person: %s, version: %s in %d ms", personId, result.version(), et);
  }
  
  @Test
  @Order(2)
  void testSavePersonAgain() throws IOException, InterruptedException {
    var saveRequest = SavePersonRequest.builder()
        .name("Harrison Ford")
