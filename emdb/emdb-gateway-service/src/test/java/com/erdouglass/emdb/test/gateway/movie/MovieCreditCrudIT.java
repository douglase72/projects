...
class MovieCreditCrudIT extends AbstractTest {
  private static final Logger LOGGER = Logger.getLogger(MovieCreditCrudIT.class);
  ...
  void testUpdateMovie() throws IOException, InterruptedException {
    ...
    var creditId = movie.credits().cast().get(1).creditId();
    var cmd = UpdateMovieCredit.of("Test Character", 3);
    request = HttpRequest.newBuilder().uri(UriBuilder
        .fromUri(MOVIES_URL).path(movie.id().toString()).path("credits").path(creditId.toString()).build())
        .header("Authorization", "Bearer " + token)
        .PUT(HttpRequest.BodyPublishers.ofString(OBJECT_MAPPER.writeValueAsString(cmd))).build();
    ...
    var credit = OBJECT_MAPPER.readValue(response.body(), MovieCreditDto.class);
    assertEquals(CreditType.CAST, credit.type());
    assertEquals("Beyoncé", credit.name());
    assertEquals(Gender.FEMALE, credit.gender());
    assertEquals("Test Character", credit.character());
    assertEquals(3, credit.order());
    LOGGER.infof("Updated movie credit %s in %d ms", creditId, et);
  }
